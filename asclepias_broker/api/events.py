...
class EventAPI:
    ...
    def create_event(cls, event):
        ...
        event_obj, errors = EventSchema(
            session=db.session, check_existing=True).load(event)
        cls.session.add(event_obj)
        return event_obj

    @classmethod
    def create_relation_object_events(cls, event, relationship, payload_idx):
        # Create the Relation entry
        rel_obj = get_or_create(db.session, ObjectEvent, event_id=event.id,
        ...
        src_obj = get_or_create(db.session, ObjectEvent, event_id=event.id,
            object_uuid=relationship.source.id,
            payload_type=PayloadType.Identifier, payload_index=payload_idx)
        tar_obj = get_or_create(db.session, ObjectEvent, event_id=event.id,
        ...
        for payload_idx, payload in enumerate(event['Payload']):
            with db.session.begin_nested():
                relationship, errors = RelationshipSchema(
                    session=db.session, check_existing=True).load(payload)
                if errors:
                ...
                relationship = relationship.fetch_or_create_id(db.session)
                cls.create_relation_object_events(event_obj, relationship, payload_idx)

                # TODO: This should be a task after the ingestion commit
                groups = update_groups(db.session, relationship)
                src_grp, tar_grp, merged_grp = groups
                # Update metadata
                update_metadata(db.session, relationship, payload)
                # Index the groups and relationships
                update_indices(db.session, src_grp, tar_grp, merged_grp)
